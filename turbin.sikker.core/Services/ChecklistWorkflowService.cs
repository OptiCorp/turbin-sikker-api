...
namespace turbin.sikker.core.Services
{
    public class ChecklistWorkflowService : IChecklistWorkflowService
    {
        ...
        {
            return await _context.ChecklistWorkflow.Include(c => c.Creator).Include(c => c.CreatedDate).Include(c => c.Checklist).ThenInclude(c => c.ChecklistTasks).FirstOrDefaultAsync();
        }

        public async Task<IEnumerable<ChecklistWorkflow>> GetAllChecklistWorkflows()
        {
            return await _context.ChecklistWorkflow.Include(c => c.Creator).Include(c => c.CreatedDate).Include(p => p.Checklist).ThenInclude(c => c.ChecklistTasks).ToListAsync();
        }

        public async Task<IEnumerable<ChecklistWorkflow>> GetAllChecklistWorkflowsByUserId(string userId)
        {
            return await _context.ChecklistWorkflow.Include(c => c.Creator).Include(c => c.CreatedDate).Include(p => p.Checklist).ThenInclude(c => c.ChecklistTasks).Where(cw => cw.UserId == userId).ToListAsync();
        }
