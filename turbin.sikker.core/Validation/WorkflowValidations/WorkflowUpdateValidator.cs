...
namespace turbin.sikker.core.Validation.WorkflowValidations
{
    public class WorkflowUpdateValidator : AbstractValidator<WorkflowUpdateDto>
    {
        public WorkflowUpdateValidator()
        {
            ...
            RuleFor(workflow => workflow.Status).Must((status) => status == "Sent" || status == "Committed" || status == "Done" || status == "Rejected" || status == null).WithMessage("Status must be Sent, Committed, Rejected or Done");
        }
    }
}
